...
### Task 1 — Update API service layer (R14, R15, R16) [x]

File: `web-ui/src/services/api/url.ts`

- Change `createURL` signature to accept an options object
  `{ originalUrl, customAlias?, expiresAt?, password?, tags?, redirectType?, utmParams? }`
  and POST the whole object to `/urls`.
- Change `updateURL` to accept the same options object (minus `originalUrl` required) and
  PUT it to `/urls/:id`.
- Change `getURLs` to accept `{ page?, limit?, search?, tag?, expired? }` and forward them
  as query params.
- Add `downloadQR(shortId: string): Promise<Blob>` that calls `GET /:shortId/qr` and
  returns the response as a Blob.
- Update `web-ui/src/services/api/url.test.ts` to cover the new signatures.

### Task 2 — Expand create form: core optional fields (R1–R5) [x]

File: `web-ui/src/app/page.tsx`

- Add **Custom alias** text input (optional). Validate: alphanumeric + hyphens, 3–50 chars,
  client-side. On 409 from the server show the error inline below the alias field.
- Add **Expiry date** `<input type="datetime-local">` (optional). Reject past dates
  client-side before submission with an inline error.
- Add **Password protection** toggle. When enabled, reveal a password `<input>`. Send as
  `password` in the request body.
- Add **Tags** input — comma-separated free text; split into a `string[]` before sending.
- Add **Redirect type** `<select>` with options 302 Temporary (default) and 301 Permanent.
  Send as `redirectType`.
- Wire all fields into the updated `createURL` call.
- All fields must have accessible `<Label>` elements and be keyboard-navigable (R18).

### Task 3 — Expand create form: advanced options + copy button (R6, R7, R17) [x]

File: `web-ui/src/app/page.tsx`

- Add a collapsible "Advanced options" section (hidden by default, toggle via button/
  disclosure) containing:
  - UTM builder fields: `utm_source`, `utm_medium`, `utm_campaign`, `utm_term`,
    `utm_content` (all optional). Assemble filled values into a `utmParams` JSON object
    before sending.
- Replace the plain short-URL anchor after creation with a result row that shows the URL
  **and** a "Copy" button that writes it to the clipboard (R7).
- Show a loading state on the Submit button while the request is in flight (R19).

### Task 4 — Manage page: pagination, search, and filters (R8–R11) [x]

File: `web-ui/src/app/manage/page.tsx`

- Drive the page with local state: `{ page, search, tag, expired }`.
- Call `getURLs({ page, limit: 10, search, tag, expired })` whenever state changes.
- Render Previous / Next buttons and a "Page X of Y (N total)" label (R8).
- Add a search `<input>` debounced 300 ms; on change reset to page 1 (R9).
- Add a **tag filter** `<select>` populated from the union of tags in the current result
  set. Selecting a tag resets to page 1 (R10).
- Add an **"Expired only"** checkbox/toggle that sets `expired=true` and resets to page 1
  (R11).
- Show a loading indicator while any fetch is in flight (R19).

### Task 5 — Manage page: QR code download (R12) [x]

File: `web-ui/src/app/manage/page.tsx`

- Add a "Download QR" button to each table row.
- On click: call `downloadQR(shortId)`, create an object URL from the returned Blob,
  trigger a synthetic `<a download="{shortId}.png">` click, then revoke the object URL.
- Show a per-row loading state while the QR fetch is in flight (R19).

### Task 6 — Manage page: modal edit form (R13) [x]

File: `web-ui/src/app/manage/page.tsx`

- Remove the existing inline edit (input in table cell + Save button).
- Add an edit modal (use shadcn `Dialog` component — install if not present) opened by the
  existing Edit button.
- The modal form exposes all editable fields: `originalUrl`, `customAlias`, `expiresAt`,
  `password`, `tags`, `redirectType`, `utmParams` (same UX as the create form fields from
  Tasks 2–3).
- Pre-populate all fields from the selected URL on open.
- On save call the updated `updateURL`; on success close the modal and refresh the row in
  state.
- Show a loading state on the Save button while the request is in flight (R19).
