...
- Add `OutboxEvent` model with fields: id (Int autoincrement PK), eventType (String),
  ...
  (DateTime?), createdAt (DateTime @default(now)).
- Create a migration file under `auth-service/prisma/migrations/`.

### Task 2 -- Wrap handlers + standalone inserts (R2, R2a, R3) [x]

Files: `auth-service/controllers/auth.controller.js`,
`auth-service/controllers/users.controller.js`

- Registration: wrap `prisma.user.create` + outbox insert in `prisma.$transaction`.
- Forgot-password: wrap `prisma.passwordResetToken.create` + outbox insert in transaction.
- Reset-password: wrap user update + token usedAt update + outbox insert in transaction.
- Password-change (`POST /users/:id/password`): wrap user update + outbox insert in transaction.
- Profile-update (`PUT /users/:id`): wrap user update + outbox insert in transaction (only
  when `updateData` is non-empty; record `changedFields` array in payload).
- User-delete (`DELETE /users/:id`): wrap user delete + outbox insert in transaction.
- Login success: insert standalone OutboxEvent after `clearAttempts`.
- Login failure: insert standalone OutboxEvent after `recordFailedAttempt` (only when
  account is not already rate-limited, i.e. the 400 branch, not the 429 branch).

### Task 3 -- Background outbox poller (R4-R8) [x]

File: `auth-service/services/outboxPoller.js` (new)

- Factory function `createOutboxPoller({ prisma, auditServiceUrl, logger })`.
- `start()`: sets an async interval every 5 s, fetches up to 100 unprocessed rows ordered
  by createdAt asc, POSTs to `${auditServiceUrl}/internal/audit-events`, marks processed.
- `stop()`: clears the interval and waits for any in-flight request to settle.
- Wire into `auth-service/index.js`: create and start the poller after RabbitMQ connects;
  stop it in `gracefulShutdown`.

### Task 4 -- Update mocks + tests (R9-R11) [x]

Files: `auth-service/test/mocks/prisma.ts`,
`auth-service/test/auth.routes.test.ts`,
`auth-service/test/users.routes.test.ts`

- Add `outboxEvent` (with `create` and `findMany`) and `$transaction` to the mock Prisma
  client. `$transaction` should execute the callback it receives.
- In `auth.routes.test.ts`: assert OutboxEvent is created with the correct eventType for
  register, login success, login failure (not when rate-limited), forgot-password (registered
  only), and reset-password.
- In `users.routes.test.ts`: assert OutboxEvent is created for password-change,
  profile-update (field change only, not no-op), and user-delete.
- Add a unit test file `auth-service/test/outboxPoller.test.ts` covering: rows forwarded
  and marked processed; unreachable audit-service leaves rows unprocessed; missing
  AUDIT_SERVICE_URL skips forwarding with a warning.
